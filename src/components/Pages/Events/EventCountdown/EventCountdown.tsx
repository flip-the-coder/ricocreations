...
const EventCountdown: React.FC<{ dateOfEvent: string; message: string | null; spanOfEvent: number | null }> = ({
    dateOfEvent,
    message,
    spanOfEvent
}) => {
    ...
    const generateDateSpanMessage = (): JSX.Element | null => {
        ...
        return <Message>{`${startDateStr} - ${eventDateStr}`}</Message>;
    };
    ...
    return (
        <Container>
            ...
            <br />
            {timerComponents.length ? <Timer>{timerComponents}</Timer> : <Timer>Time's up!</Timer>}
        </Container>
    );
};
