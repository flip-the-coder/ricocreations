...
export const SERVER_CLIENT_TIMESTAMP_MISMATCH = 'Server/client timestamp mismatch.';
...
export function handleError(error: unknown): string {
    ...
    if (Transport.isAxiosError(error)) {
        ...
        if (serverError.response) {
            ...
            if (responseData.error_description) {
                if (responseData.relatedMessages) {
                    ...
                    const isTimestampMismatch =
                        relatedMessages.filter((m: string) => m.includes(SERVER_CLIENT_TIMESTAMP_MISMATCH)).length > 0;
            ...
            } else if (Array.isArray(responseData) && responseData.length > 0 && responseData[0].customMessage) {
