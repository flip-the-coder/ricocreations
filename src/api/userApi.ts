...
const account = {
   requestPasswordResetLink: (email: string) => {
      ...
      return Transport.post(
         `${USER_API}/account/requestpasswordreset`,
         { email },
         undefined,
         config
      );
   ...
   resetPassword: (
      ...
      return Transport.post(
         `${USER_API}/account/resetpassword?userId=${userId}&code=${token}`,
         {
   ...
   verifyEmail: (userId: string, token: string) => {
      return Transport.post(
         `${USER_API}/account/confirmemail?userId=${userId}&code=${token}`
      );
   },
   resendEmailConfirmation: (email: string) => {
      ...
      return Transport.post(
         `${USER_API}/account/resendemailconfirmation`,
         { email },
         undefined,
         config
      );
   ...
   registerToken: (
      registerModel: UserRegistrationWithTokenModel,
      token: string
   ) => {
      const config = {
         headers: {
...
const userRegistrationToken = {
   validateToken: (token: string): AxiosPromise => {
      return Transport.get(
         `${USER_API}/userregistrationtoken/${token}/validate`
      );
   },
...
const clientCredentials = {
   ...
   validateToken: (scopes: string[]): AxiosPromise => {
      return Transport.post(
         `${USER_API}/clientcredentials/token/validate`,
         scopes
      );
   },
};
